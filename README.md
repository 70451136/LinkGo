...
LinkEngine 负责文件翻译工作台的任务编排、多格式文件管道、术语记忆上下文和交付质量门。

![LinkEngine 文件翻译工作台架构](docs/assets/architecture/linkengine-architecture.png)

LinkClaw 负责把 LinkGo 的模型、术语、记忆、文件管道和安全边界桥接给 OpenClaw / Hermes Agent，让本机智能体任务回到 LinkGo 的可观测交付链路。

![LinkClaw 本机智能体桥接架构](docs/assets/architecture/linkclaw-architecture.png)
...
- [LinkClaw 工作流图册 / Workflow Gallery](docs/LINKCLAW_WORKFLOW_GALLERY.md)
...
LinkClaw 把本机运行时、OpenClaw / Hermes Agent、文件管道、模型选择和沙箱权限整合到 LinkGo 的工作台里。完整截图说明见 [LinkClaw 工作流图册](docs/LINKCLAW_WORKFLOW_GALLERY.md)。

![LinkClaw 运行时面板](assets/linkclaw/linkclaw-runtime-dashboard.png)

![PPT 保留格式翻译完成摘要](assets/linkclaw/ppt-korean-translation-summary.png)
...
LinkClaw brings local runtimes, OpenClaw / Hermes Agent, file pipelines, model selection, and sandbox permissions into the LinkGo workspace. See the full bilingual screenshot walkthrough in [LinkClaw Workflow Gallery](docs/LINKCLAW_WORKFLOW_GALLERY.md).
